...
router.get('/tickets', (req, res) => {
  ...
  const avgResolution = db.prepare(`
    ...
    FROM tickets
    WHERE resolved_at IS NOT NULL AND created_at >= date('now', '-${parseInt(period)} days')
  `).get();
  ...
  const topResolvers = db.prepare(`
    ...
    WHERE t.resolved_at IS NOT NULL AND t.created_at >= date('now', '-${parseInt(period)} days')
  ...
  const performance = db.prepare(`
    SELECT u.id, u.first_name || ' ' || u.last_name as name, u.role,
      ...
      (SELECT COUNT(*) FROM tickets WHERE assigned_to = u.id AND resolved_at IS NOT NULL 
        AND resolved_at >= date('now', '-${parseInt(period)} days')) as resolved_tickets,
      (SELECT AVG(julianday(resolved_at) - julianday(created_at)) 
        FROM tickets WHERE assigned_to = u.id AND resolved_at IS NOT NULL
        AND resolved_at >= date('now', '-${parseInt(period)} days')) as avg_resolution_days,
      (SELECT COUNT(*) FROM project_tasks WHERE assigned_to = u.id AND status = 'done'
        AND completed_at >= date('now', '-${parseInt(period)} days')) as completed_tasks
    FROM users u
    WHERE u.is_active = 1
