...
describe('templates render without ReferenceError', () => {
  ...
  it('dashboard renders all five dynamic list sections (not just My Tickets)', () => {
    const html = render('dashboard.ejs', {
      ...
      assetStats: { total: 4, in_use: 2, in_storage: 1, in_repair: 1, reserved: 0 },
...
describe('every template renders without error (regression)', () => {
  ...
  const fixtures = [
    ...
    { name: 'vendors/form (edit)', file: 'vendors/form.ejs', locals: () => ({ ...baseLocals(), title: 'Edit Vendor', vendor: { id: 1, name: 'Acme Corp', contact_person: 'John', email: 'john@acme.com', phone: '555-0000', address: '123 Main St', website: 'https://acme.com', category: 'hardware', contract_start: '2024-01-01', contract_end: '2025-01-01', rating: 4, is_active: 1, notes: 'Preferred vendor' }, isEdit: true }) },
    ...
    { name: 'licenses/form (edit)', file: 'licenses/form.ejs', locals: () => ({ ...baseLocals(), title: 'Edit License', license: { id: 1, software_name: 'Microsoft 365', vendor: 'Microsoft', license_key: 'MS365-KEY-1234', license_type: 'subscription', total_seats: 200, used_seats: 187, purchase_date: '2025-01-01', expiry_date: '2026-01-01', cost: 28000, notes: 'Annual subscription' }, isEdit: true }) },
    ...
    { name: 'projects/form (edit)', file: 'projects/form.ejs', locals: () => ({ ...baseLocals(), title: 'Edit Project', project: { id: 1, name: 'Cloud Migration', description: 'Migrate to cloud', status: 'in_progress', priority: 'high', start_date: '2026-01-01', end_date: '2026-12-31', budget: 100000, spent: 25000, progress: 25, owner_id: 2, owner_name: 'Admin User', created_at: '2026-01-01' }, staff: [staffUser], isEdit: true }) },
    ...
    { name: 'changes/form (edit)', file: 'changes/form.ejs', locals: () => ({ ...baseLocals(), title: 'Edit Change', isEdit: true, change: { id: 1, title: 'Server Patch', change_type: 'maintenance', status: 'scheduled', priority: 'high', impact: 'Minimal downtime', description: 'Apply security patches', scheduled_start: '2026-05-01 02:00', scheduled_end: '2026-05-01 04:00', actual_start: null, actual_end: null, assigned_to: 3, assigned_name: 'Maya Patel' }, staff: [staffUser] }) },
    ...
    { name: 'assets/form (edit)', file: 'assets/form.ejs', locals: () => ({ ...baseLocals(), title: 'Edit Asset', asset: { id: 1, name: 'MacBook Pro', asset_tag: 'AST-001', category: 'laptop', manufacturer: 'Apple', model: 'M3 Max', serial_number: 'SN-001', status: 'in_use', condition_rating: 'good', purchase_date: '2024-01-15', purchase_price: 3499, warranty_expiry: '2027-01-15', assigned_to: 1, location: 'HQ Office', notes: 'Company laptop', assigned_name: 'Sarah Chen', assigned_email: 'admin@company.com' }, staff: [staffUser], isEdit: true }) },
